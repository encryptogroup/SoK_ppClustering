...
// Updated header file path (modified from original)
#include "HEAAN.h"
/**
 * This file is for test HEAAN library
 * You can find more in src/TestScheme.h
 * "./TestHEAAN Encrypt" will run Encrypt Test
 * There are Encrypt, EncryptSingle, Add, Mult, iMult, RotateFast, Conjugate
 * Tests
 */
int main(int argc, char **argv) {
  ...
  if (string(argv[1]) == "EncryptSingle")
  ...
  if (string(argv[1]) == "RotateFast")
    TestScheme::testRotateFast(logq, logp, logn, r);
  if (string(argv[1]) == "Conjugate")
  ...
  logq = logp + 10;  //< suppose the input ciphertext of bootstrapping has logq
                     //= logp + 10
