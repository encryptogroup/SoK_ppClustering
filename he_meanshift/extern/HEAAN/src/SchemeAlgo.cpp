...
void SchemeAlgo::powerOf2Extended(Ciphertext* res, Ciphertext& cipher,
                                  long logp, long logDegree) {
  res[0].copy(cipher);
...
void SchemeAlgo::power(Ciphertext& res, Ciphertext& cipher, long logp,
                       long degree) {
...
void SchemeAlgo::powerExtended(Ciphertext* res, Ciphertext& cipher, long logp,
                               long degree) {
  long logDegree = log2((double)degree);
...
void SchemeAlgo::inverse(Ciphertext& res, Ciphertext& cipher, long logp,
                         long steps) {
  Ciphertext cbar;
  Ciphertext cpow;
  Ciphertext tmp;
...
void SchemeAlgo::function(Ciphertext& res, Ciphertext& cipher, string& funcName,
                          long logp, long degree) {
...
void SchemeAlgo::functionLazy(Ciphertext& res, Ciphertext& cipher,
                              string& funcName, long logp, long degree) {
  Ciphertext* cpows = new Ciphertext[degree];
  powerExtended(cpows, cipher, logp, degree);
