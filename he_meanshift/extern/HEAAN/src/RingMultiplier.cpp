...
RingMultiplier::RingMultiplier() {
  ...
  for (long i = 0; i < nprimes; ++i) {
    ...
    for (long j = 0; j < N; ++j) {
      ...
      mulMod(scaledRootPows[i][jprime], scaledRootPows[i][jprime], (1ULL << 32),
             pVec[i]);
      uint64_t rootpowInv = powerInv;
      mulMod(scaledRootInvPows[i][jprime], rootpowInv, (1ULL << 32), pVec[i]);
      mulMod(scaledRootInvPows[i][jprime], scaledRootInvPows[i][jprime],
             (1ULL << 32), pVec[i]);
...
void RingMultiplier::multNTT(ZZ* x, ZZ* a, uint64_t* rb, long np,
                             const ZZ& mod) {
  uint64_t* ra = new uint64_t[np << logN]();
  uint64_t* rx = new uint64_t[np << logN]();
  NTL_EXEC_RANGE(np, first, last);
...
void RingMultiplier::multDNTT(ZZ* x, uint64_t* ra, uint64_t* rb, long np,
                              const ZZ& mod) {
  uint64_t* rx = new uint64_t[np << logN]();
...
void RingMultiplier::multNTTAndEqual(ZZ* a, uint64_t* rb, long np,
                                     const ZZ& mod) {
  uint64_t* ra = new uint64_t[np << logN]();
...
void RingMultiplier::mulModBarrett(uint64_t& r, uint64_t a, uint64_t b,
                                   uint64_t p, uint64_t pr) {
  unsigned __int128 mul = static_cast<unsigned __int128>(a) * b;
...
void RingMultiplier::butt(uint64_t& a, uint64_t& b, uint64_t W, uint64_t p,
                          uint64_t pInv) {
...
void RingMultiplier::ibutt(uint64_t& a, uint64_t& b, uint64_t W, uint64_t p,
                           uint64_t pInv) {
...
void RingMultiplier::idivN(uint64_t& a, uint64_t NScale, uint64_t p,
                           uint64_t pInv) {
