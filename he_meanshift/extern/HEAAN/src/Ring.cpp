...
void Ring::decode(ZZ* mx, complex<double>* vals, long slots, long logp,
                  long logq) {
  ZZ q = qpows[logq];
  long gap = Nh / slots;
  ZZ tmp;
...
void Ring::addBootContext(long logSlots, long logp) {
  if (bootContextMap.find(logSlots) == bootContextMap.end()) {
    ...
    bootContextMap.insert(pair<long, BootContext*>(
        logSlots, new BootContext(rpvec, rpvecInv, rp1, rp2, bndvec, bndvecInv,
                                  bnd1, bnd2, logp)));
  }
}
