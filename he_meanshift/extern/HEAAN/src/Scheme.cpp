...
Scheme::~Scheme() {
  for (auto const& t : keyMap) delete t.second;
  for (auto const& t : leftRotKeyMap) delete t.second;
}
...
void Scheme::addRightRotKey(SecretKey& secretKey, long r) {
  ...
  if (leftRotKeyMap.find(idx) == leftRotKeyMap.end() &&
      serLeftRotKeyMap.find(idx) == serLeftRotKeyMap.end()) {
...
void Scheme::addBootKey(SecretKey& secretKey, long logl, long logp) {
  ...
  for (long i = 1; i < k; ++i) {
    if (leftRotKeyMap.find(i) == leftRotKeyMap.end() &&
        serLeftRotKeyMap.find(i) == serLeftRotKeyMap.end()) {
...
void Scheme::encode(Plaintext& plain, double* vals, long n, long logp,
                    long logq) {
  plain.logp = logp;
  plain.logq = logq;
  plain.n = n;
...
void Scheme::encode(Plaintext& plain, complex<double>* vals, long n, long logp,
                    long logq) {
  plain.logp = logp;
  plain.logq = logq;
  plain.n = n;
...
void Scheme::encodeSingle(Plaintext& plain, complex<double> val, long logp,
                          long logq) {
  plain.logp = logp;
  plain.logq = logq;
  plain.n = 1;
...
void Scheme::encrypt(Ciphertext& cipher, complex<double>* vals, long n,
                     long logp, long logq) {
  Plaintext plain;
...
void Scheme::encrypt(Ciphertext& cipher, double* vals, long n, long logp,
                     long logq) {
  Plaintext plain;
  encode(plain, vals, n, logp, logq);
  encryptMsg(cipher, plain);
...
void Scheme::encryptSingle(Ciphertext& cipher, complex<double> val, long logp,
                           long logq) {
  Plaintext plain;
...
void Scheme::encryptSingle(Ciphertext& cipher, double val, long logp,
                           long logq) {
  Plaintext plain;
  encodeSingle(plain, val, logp, logq);
  encryptMsg(cipher, plain);
...
void Scheme::addConst(Ciphertext& res, Ciphertext& cipher, double cnst,
                      long logp) {
  ZZ q = ring.qpows[cipher.logq];
  ZZ cnstZZ = logp < 0 ? EvaluatorUtils::scaleUpToZZ(cnst, cipher.logp)
                       : EvaluatorUtils::scaleUpToZZ(cnst, logp);
  res.copy(cipher);
  AddMod(res.bx[0], res.bx[0], cnstZZ, q);
}

void Scheme::addConst(Ciphertext& res, Ciphertext& cipher, RR& cnst,
                      long logp) {
  ZZ q = ring.qpows[cipher.logq];
  ZZ cnstZZ = logp < 0 ? EvaluatorUtils::scaleUpToZZ(cnst, cipher.logp)
                       : EvaluatorUtils::scaleUpToZZ(cnst, logp);
...
void Scheme::addConst(Ciphertext& res, Ciphertext& cipher, complex<double> cnst,
                      long logp) {
  ZZ q = ring.qpows[cipher.logq];
  ZZ cnstZZ = logp < 0 ? EvaluatorUtils::scaleUpToZZ(cnst.real(), cipher.logp)
                       : EvaluatorUtils::scaleUpToZZ(cnst.real(), logp);
  res.copy(cipher);
...
void Scheme::addConstAndEqual(Ciphertext& cipher, complex<double> cnst,
                              long logp) {
  ZZ q = ring.qpows[cipher.logq];
  ZZ cnstrZZ = logp < 0 ? EvaluatorUtils::scaleUpToZZ(cnst.real(), cipher.logp)
                        : EvaluatorUtils::scaleUpToZZ(cnst.real(), logp);
  ZZ cnstiZZ = logp < 0 ? EvaluatorUtils::scaleUpToZZ(cnst.imag(), cipher.logp)
                        : EvaluatorUtils::scaleUpToZZ(cnst.imag(), logp);
...
void Scheme::multByConst(Ciphertext& res, Ciphertext& cipher, double cnst,
                         long logp) {
  ZZ q = ring.qpows[cipher.logq];
...
void Scheme::multByConst(Ciphertext& res, Ciphertext& cipher,
                         complex<double> cnst, long logp) {
  res.copy(cipher);
  multByConstAndEqual(res, cnst, logp);
}

void Scheme::multByConstVec(Ciphertext& res, Ciphertext& cipher,
                            complex<double>* cnstVec, long logp) {
  res.copy(cipher);
  multByConstVecAndEqual(res, cnstVec, logp);
}

void Scheme::multByConstVecAndEqual(Ciphertext& cipher,
                                    complex<double>* cnstVec, long logp) {
  long slots = cipher.n;
...
void Scheme::multByPoly(Ciphertext& res, Ciphertext& cipher, ZZ* poly,
                        long logp) {
  ZZ q = ring.qpows[cipher.logq];
  res.copyParams(cipher);
...
void Scheme::multByPolyNTT(Ciphertext& res, Ciphertext& cipher, uint64_t* rpoly,
                           long bnd, long logp) {
  ZZ q = ring.qpows[cipher.logq];
  res.copyParams(cipher);
  long np = ceil((cipher.logq + bnd + logN + 2) / (double)pbnd);
...
void Scheme::multByPolyNTTAndEqual(Ciphertext& cipher, uint64_t* rpoly,
                                   long bnd, long logp) {
  ZZ q = ring.qpows[cipher.logq];
  long np = ceil((cipher.logq + bnd + logN + 2) / (double)pbnd);
  ring.multNTTAndEqual(cipher.ax, rpoly, np, q);
...
void Scheme::multByMonomial(Ciphertext& res, Ciphertext& cipher,
                            const long degree) {
  res.copyParams(cipher);
...
void Scheme::coeffToSlotAndEqual(Ciphertext& cipher) {
  ...
  for (long j = first; j < last; ++j) {
    multByPolyNTT(tmpvec[j], rotvec[j], bootContext->rpvec[j],
                  bootContext->bndvec[j], bootContext->logp);
  }
  NTL_EXEC_RANGE_END;
  ...
  for (long ki = k; ki < slots; ki += k) {
    NTL_EXEC_RANGE(k, first, last);
    for (long j = first; j < last; ++j) {
      multByPolyNTT(tmpvec[j], rotvec[j], bootContext->rpvec[j + ki],
                    bootContext->bndvec[j + ki], bootContext->logp);
    }
    NTL_EXEC_RANGE_END;
    for (long j = 1; j < k; ++j) {
      addAndEqual(tmpvec[0], tmpvec[j]);
...
void Scheme::slotToCoeffAndEqual(Ciphertext& cipher) {
  ...
  NTL_EXEC_RANGE(k, first, last);
  for (long j = first; j < last; ++j) {
    multByPolyNTT(tmpvec[j], rotvec[j], bootContext->rpvecInv[j],
                  bootContext->bndvecInv[j], bootContext->logp);
  }
  NTL_EXEC_RANGE_END;
  ...
  for (long ki = k; ki < slots; ki += k) {
    NTL_EXEC_RANGE(k, first, last);
    for (long j = first; j < last; ++j) {
      multByPolyNTT(tmpvec[j], rotvec[j], bootContext->rpvecInv[j + ki],
                    bootContext->bndvecInv[j + ki], bootContext->logp);
    }
    NTL_EXEC_RANGE_END;
...
void Scheme::evalExpAndEqual(Ciphertext& cipher, long logT, long logI) {
  ...
  if (logSlots < logNh) {
    ...
    exp2piAndEqual(cipher,
                   bootContext->logp);  // bitDown: logT + 1 + 3(logq + logI)
    ...
    conjugate(tmp, cipher);
    subAndEqual(cipher, tmp);
    multByPolyNTT(tmp, cipher, bootContext->rp1, bootContext->bnd1,
                  bootContext->logp);
    Ciphertext tmprot;
    leftRotateFast(tmprot, tmp, slots);
    addAndEqual(tmp, tmprot);
    multByPolyNTTAndEqual(cipher, bootContext->rp2, bootContext->bnd2,
                          bootContext->logp);
    ...
    exp2piAndEqual(
        cipher,
        bootContext->logp);  // cipher bitDown: logT + 1 + 3(logq + logI)
    exp2piAndEqual(c2,
                   bootContext->logp);  // c2 bitDown: logT + 1 + 3(logq + logI)
    for (long i = 0; i < logI + logT; ++i) {
      squareAndEqual(c2);
      squareAndEqual(cipher);
...
void Scheme::bootstrapAndEqual(Ciphertext& cipher, long logq, long logQ,
  ...
  evalExpAndEqual(cipher, logT,
                  logI);  // bitDown: context.logNh + (logI + logT + 5) * logq +
                          // (logI + logT + 6) * logI + logT + 1
